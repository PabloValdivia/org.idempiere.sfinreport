...
//	private static final long serialVersionUID = 3463776496724974142L;
	 ...
	 * 	@throws Exception
	 */
	 public SfrReportViewer(final Integer idReport, final String nameTrx, final Integer idReportLineSet, final Integer C_Period_ID, final Integer AD_PrintFont_ID, final MReportColumn[] columns,ReportEngine re,int orgTrx_id) {
	    	super();
	    	MOrgInfo oi = new Query(Env.getCtx(),MOrgInfo.Table_Name,"AD_Org_ID = ?",null).setParameters(orgTrx_id).first();
	    	if (oi!=null)
...
//	        }
//	        catch (Exception e) {
//	        	SfrReportViewer.log.log(Level.SEVERE, "", (Throwable)e);
//	            FDialog.error(this.m_WindowNo, (Component)this, "LoadError", e.getLocalizedMessage());
//	            this.onClose();
//	        }
	    }
	 
	public SfrReportViewer(ReportEngine re, String title) {		
		super();
	...
	private void dynInit()
	{
        ...
        this.data = (LinkedList<ReportTO>)logic.getDataReport(reportId,trxName);
        this.generalTitle = logic.getGeneralTitle(this.reportLineSetId, trxName);
	...
	private void hideBusyDialog() {
	 ...
	 private void createXlsReport() {
	        ...
	        final HSSFWorkbook book = xls.generate((LinkedList)data, generalTitle, clientName, clientNIT, periodName, currencyName, m_columns, city, logoId);
			if (log.isLoggable(Level.FINE))
			{
				log.log(Level.FINE, "this.generalTitle="+this.generalTitle + " generalTitle[0] ="+generalTitle[0] );
			}
	...
	static class PDFRendererRunnable extends ContextRunnable implements IServerPushCallback {
		...
		public PDFRendererRunnable(SfrReportViewer viewer) {
			super();
			this.viewer = viewer;
		}
		...
		protected void doRun() {
			try {
				...
				viewer.media = new AMedia(filePdf.getName(), "pdf", "application/pdf", filePdf, true);
			} catch (Exception e) {
	...
	static class HTMLRendererRunnable extends ContextRunnable implements IServerPushCallback {
		...
		public HTMLRendererRunnable(SfrReportViewer viewer) {
			super();
			this.viewer = viewer;
	...
	static class XLSRendererRunnable extends ContextRunnable  implements IServerPushCallback {
		...
		public XLSRendererRunnable(SfrReportViewer viewer) {
			super();
			this.viewer = viewer;
		}
		...
		protected void doRun() {
			try {
				...
				viewer.media = new AMedia(file.getName(), "xls", "application/vnd.ms-excel",fileXls , true);
			} catch (Exception e) {
				if (e instanceof RuntimeException)
					throw (RuntimeException)e;
				else
